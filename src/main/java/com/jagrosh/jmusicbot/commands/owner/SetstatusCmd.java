...
public class SetstatusCmd extends OwnerCommand
{
    ...
    {
        this.name = "تعيين_الحالة";
        this.help = "يحدد حالة ظهور البوت";
    ...
    protected void execute(CommandEvent event) 
    {
        try {
            ...
            {
                event.replyError("يرجى إدخال إحدى الحالات التالية: `ONLINE`, `IDLE`, `DND`, `INVISIBLE`");
            }
            else
            {
                event.getJDA().getPresence().setStatus(status);
                event.replySuccess("تم تعيين الحالة إلى `"+status.getKey().toUpperCase()+"`");
            }
        } catch(Exception e) {
