...
public class ForceRemoveCmd extends DJCommand
{
    ...
    {
        super(bot);
        this.name = "ازالة_اجبارية";
        this.help = "يحذف جميع المقاطع التي أضافها مستخدم معين من القائمة";
        this.arguments = "<user>";
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyError("يجب أن تذكر مستخدمًا!");
            return;
        }
        ...
        {
            event.replyError("لا يوجد شيء في القائمة!");
            return;
        }
        ...
        if(found.isEmpty())
        {
            event.replyError("تعذر العثور على المستخدم!");
            return;
        }
        else if(found.size()>1)
        {
            ...
            .setText("تم العثور على عدة مستخدمين:")
    ...
    private void removeAllEntries(User target, CommandEvent event)
    {
        ...
        {
            event.replySuccess("تمت إزالة `"+count+"` entries from "+FormatUtil.formatUsername(target)+".");
        }
    }
}
