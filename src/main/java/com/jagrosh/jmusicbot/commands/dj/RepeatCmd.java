...
public class RepeatCmd extends DJCommand
{
    public RepeatCmd(Bot bot)
    {
        super(bot);
        this.name = "تكرار";
        this.help = "يعيد إضافة الموسيقى للقائمة عند الانتهاء منها";
    ...
    protected void execute(CommandEvent event) 
    {
        ...
        {
            event.replyError("الخيارات المتاحة هي `off` أو `all` أو `single` (أو اتركه فارغًا للتبديل بين `off` و`all`)");
            return;
        }
        settings.setRepeatMode(value);
        event.replySuccess("وضع التكرار الآن `"+value.getUserFriendlyName()+"`");
    }
