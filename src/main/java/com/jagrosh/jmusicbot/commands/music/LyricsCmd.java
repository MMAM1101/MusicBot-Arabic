...
public class LyricsCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        if(event.getArgs().isEmpty())
        {
            ...
            {
                event.replyError("يجب أن تكون هناك موسيقى قيد التشغيل لاستخدام هذا!");
                return;
            }
        }
        else
        ...
        client.getLyrics(title).thenAccept(lyrics -> 
        {
            if(lyrics == null)
            {
                event.replyError("كلمات أغنية `" + title + "` could not be found!" + (event.getArgs().isEmpty() ? " Try entering the song name manually (`lyrics [song name]`)" : ""));
                return;
            }
            ...
            {
                event.replyWarning("كلمات أغنية `" + title + "` found but likely not correct: " + lyrics.getURL());
            }
